d3 = function() {
  ...
  try {
    ...
    console.log(error, error[0], error[1], ' error ', typeof(error));
    var d3_style_prototype = d3_window.CSSStyleDeclaration.prototype, d3_style_setProperty = d3_style_prototype.setProperty;
    d3_style_prototype.setProperty = function(name, value, priority) {
      if (value == 'NaNpx') {value = 0};
  ...
  // console.log(target.id)
          ...
          if (allNodes[0][allNodes[0].length-1].id != target.id) {
            allNodes.sort(
                function(a,b) {
                  // console.log(a.name)
                    if (a.name == target.id) {
          ...
          if (d3.select('.pop-up')[0][0] != undefined || d3.select('.pop-up')[0][0] != null) {
              var popup = d3.select('.pop-up.' + target.id),
                translateArray = target.attributes[2]['value'].split(",");
              if (!translateArray[1]) {
                translateArray = target.attributes[2]['value'].split(" ");
              };
              var translateX = translateArray[0].match(/(-?)(\d+).\d+/g) || translateArray[0].match(/(-?)(\d+)/g),
                translateY = translateArray[1].match(/(-?)(\d+).\d+/g) || translateArray[1].match(/(-?)(\d+)/g),
                currentScale = d3.select('#map-svg')[0][0]['scale'],
                popX = (translateX - popup[0][0]['nodex'])*currentScale + popup[0][0]['offsetx'] +'px',
                popY = (translateY - popup[0][0]['nodey'])*currentScale + popup[0][0]['offsety'] +'px';

              // console.log(popup[0][0])
              // console.log(currentScale, translateX, popup[0][0]['nodex'], popup[0][0]['offsetx'], popX);
              popup
                .style('left', popX)
                .style('top', popY)
            var tar = d3.select(target)  
            if (tar.classed('waiting')) {
              tar
                .classed('waiting', false)
                .classed('dragging', true)
          //mouseover function is regained once mouseout is called on target node
  ...
  ///////
          ...
          moved |= dx | dy;
          // console.log(p, origin_)
          origin_ = p;
          d3_eventCancel();
          if (target.className.baseVal.indexOf("orbit") > -1) {
            event_({
              type: "drag",
              x: p[0] + offset[0],
              y: p[1] + offset[1],
...
////////
  ...
  function d3_selection_attr(name, value) {
    ...
    function attrFunction() {
      ...
      if (x == null) this.removeAttribute(name);
        ...
        // if (!x) {
        //   console.log(name, x, this, arguments)
  ...
  d3_selectionPrototype.each = function(callback) {
    return d3_selection_each(this, function(node, i, j) {
      // if (i == 93) { console.log(node.__data__, i, j)}
...
////      // .on("dblclick.zoom", dblclick)
      .on("touchstart.zoom", touchstart).on("touchmove.zoom", touchmove).on("touchend.zoom", touchstart);
    }
